...
do 
  ...
  for c=classes'
    ...
    %vector suma de z de cada componente vector 1xk
    sumz = sum(z);
    %probabilidad a priori de todas las componentes (k) dada la clase c
    pkGc{ic} = sumz/Nc;
    %vector de medias para todas las componentes (k) dada la clase c vector DxN NxK = DxK
    mu{ic} = (Xc' * z)./sumz;
    %vector de covarianzas
    for k=1:K
        matcov = ((Xc-mu{ic}(:,k)')' * (z(:,k).*(Xc-mu{ic}(:,k)')))/sumz(k);
        % D x D   =    D x N              N x D
        %suavizado de matriz de covariance mediante flat smoothing
